...
import ParivaarHero from "@/components/parivaar/ParivaarHero";
import ParivaarTiers from "@/components/parivaar/ParivaarTiers";
import MyDivineJourneyStats from "@/components/parivaar/MyDivineJourneyStats";
import ParivaarBlessing from "@/components/parivaar/ParivaarBlessing";
import FamilyWall from "@/components/parivaar/FamilyWall";
import CelebrateWithUs from "@/components/parivaar/CelebrateWithUs";

export default function PremSevaParivaarPage() {
  return (
    <main className="min-h-screen w-full bg-[#FFFBF4] relative overflow-hidden pb-12 pt-32">
      ...
      <div className="fixed inset-0 pointer-events-none select-none z-0">
        ...
        <motion.div 
          ...
          className="absolute w-[80%] h-[80%] top-[10%] left-[-10%] bg-radial from-rose-500/10 via-[#FFF3DF]/40 to-transparent filter blur-[100px]" 
        />
        
        {/* Subtle floating petals */}
        {Array.from({ length: 12 }).map((_, i) => (
          <motion.div
            key={`petal-parivaar-${i}`}
      ...
      <div className="w-full max-w-6xl mx-auto px-6 mb-4 relative z-50">
        <div className="flex flex-wrap items-center gap-2 text-[9px] uppercase tracking-widest font-bold mb-8">
          ...
          <span className="text-charcoal/70">Prem Seva Parivaar</span>
          <ChevronRight className="w-3 h-3 text-gold-start/50" />
